...
class CopyCommand(BaseCommand):
    ...
    def execute(self) -> None:
        ...
        if self.file_exists(destination, source_base):
            if not(overwrite_file or print_moving_log):
                ...
                print(f'cp: copying \'{source_base}\' to \'{os.path.basename(destination_base)}\'')
                if overwrite_file:
            ...
            shutil.copy(os.path.join(self.current_path, self.src), destination)
            if print_moving_log:
                print(f'cp: copying \'{source_base}\' to \'{destination_base}\'')
